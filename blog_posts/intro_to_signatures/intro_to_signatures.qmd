...
*Referencing this post: Landy, Jenna (2024, February 9), Getting Started with Mutational Signatures, Learning the Curve.*
...
For SBSs, a "type" of mutation is defined by the change in the sequence and the nucleotides on either side. For instance, a sequence ACT mutated to AGT has a C\>T substitution (the mutation) with an A on the left and a T on the right (the context). We represent this mutation type as "A\[C\>G\]T".
